...
注意: 这个脚本假设 AirSim 已经跑起来了，而且 settings.json 可以动态修改
实际跑的时候可能需要手动重启 AirSim 切换无人机数量，有点麻烦
"""
...
# settings.json 模板，n_drones 个无人机，圆形排布
# 参考: https://github.com/Midas75/AirSim/issues/1
...
def write_settings(n: int):
    ...
    print(f"[info] settings.json 已写入 ({n} 架无人机)")
...
def main():
    ...
    ap.add_argument("--models", type=str, default="M1,M2,M4",
                    help="逗号分隔，如 M1,M2,M4")
    ap.add_argument("--duration", type=int, default=20)
    ap.add_argument("--output", type=str, default="results/data")
    ap.add_argument("--write_settings", action="store_true",
                    help="自动写 settings.json（每次改完需要重启 AirSim）")
    args = ap.parse_args()

    models = [m.strip() for m in args.models.split(",")]
    ns = [1, 5, 10, 15, 20, 25, args.max_drones]
    ns = sorted(set(n for n in ns if n <= args.max_drones))

    print(f"计划: models={models}  ns={ns}  dur={args.duration}s")
    print(f"共 {len(models)*len(ns)} 组实验\n")

    for n in ns:
        if args.write_settings:
            write_settings(n)
            print(f"[info] 请重启 AirSim 以加载 {n} 架无人机配置，然后按回车继续...")
            input()

        for m in models:
            run_test(n, m, args.duration, args.output)
            time.sleep(3)  # 两组之间留点时间
