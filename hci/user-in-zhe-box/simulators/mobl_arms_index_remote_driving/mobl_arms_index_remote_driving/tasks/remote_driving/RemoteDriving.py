...
class RemoteDriving(BaseTask):
  ...
  def __init__(self, model, data, end_effector, **kwargs):
    ...
    # Terminal velocity -- car needs to stay inside target area (i.e., velocity along the track needs to fall below
    # some threshold). Default must be > 0: with 0.0, |v| <= 0 is almost never satisfied in floating-point physics.
    self._car_velocity_threshold = kwargs.get('car_velocity_threshold', 0.05)
