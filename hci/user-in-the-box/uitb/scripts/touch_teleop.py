#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""小物体触碰：辅助姿态 + 方向键操控手指触碰小目标。

适用于 SmallObjectTouch 任务。手臂从参考姿态出发，方向键控制手指在目标平面内持续平移，
松手后保持当前位置。
...
def _draw_hud(surface: pygame.Surface, lines: list[str], color_hint: tuple = (255, 255, 255)) -> None:
  try:
  ...
  y = 6
  for txt in lines:
    rendered = font.render(txt, True, color_hint)
...
def _parse_args() -> argparse.Namespace:
  p = argparse.ArgumentParser(description="SmallObjectTouch teleop with assisted pose.")
  ...
  p.add_argument("--layout", choices=("wasd", "arrows"), default="wasd")
  return p.parse_args()
...
def main() -> int:
  ...
  if task_name != "SmallObjectTouch":
    print(f"本脚本仅用于 SmallObjectTouch 任务。当前: {task_name}", file=sys.stderr)
    sim.close()
    return 1
  ...
  print(f"小物体触碰遥控（辅助姿态）：{args.layout.upper()} 控制手指  Shift 加速  R 重置  Home 回中  ESC 退出")
  ...
  try:
    while running:
      ...
      t_radius = float(state.get("target_radius", 0.03))
      contact = bool(state.get("object_contact", False))
      ...
      if flash_timer > 0:
        flash_timer -= 1
      hud_color = (0, 255, 80) if flash_timer > 0 else (255, 255, 255)
      ...
      hud = [
          ...
          f"inside={inside}  contact={contact}  hits={targets_hit}",
          f"pos_y={off_y:+.2f}  pos_z={off_z:+.2f}  r={reward:.3f}",
          f"hold={'ON' if args.return_rate <= 0.0 else 'OFF'}  focus={'YES' if focus_ok else 'NO '}  keys={key_text}",
      ...
      if sim._render_window is not None:
        _draw_hud(pygame.display.get_surface(), hud, hud_color)
        pygame.display.flip()
