...
class MyoLegsIm(MyoLegsTask):
    ...
    def compute_reward(self, action: Optional[np.ndarray] = None) -> float:
        ...
        joint_angle_reward = self.compute_joint_angle_reward(ref_dict)
        reward += joint_angle_reward * self.reward_specs.get("w_joint_angle", 0.0)

        muscle_drive_reward = self.compute_muscle_drive_reward()
        reward += muscle_drive_reward * self.reward_specs.get("w_muscle_drive", 0.0)

        smoothness_reward = self.compute_smoothness_reward(action)
        reward += smoothness_reward * self.reward_specs.get("w_smoothness", 0.0)

        self.reward_info = reward_raw
        self.reward_info["upright_reward"] = upright_reward
        self.reward_info["energy_reward"] = energy_reward
        self.reward_info["joint_angle_reward"] = joint_angle_reward
        self.reward_info["muscle_drive_reward"] = muscle_drive_reward
        self.reward_info["smoothness_reward"] = smoothness_reward
    ...
    def compute_joint_angle_reward(self, ref_dict: dict) -> float:
        """
        Rewards matching the reference joint angles.

        This term complements body-position imitation by penalizing internal joint
        angle deviations, which is closer to the thesis optimization objective than
        tracking Cartesian body points alone.
        """
        ref_qpos = getattr(ref_dict, "qpos", None)
        if ref_qpos is None:
            return 0.0

        current_qpos = np.asarray(self.get_qpos()[7:], dtype=np.float64).reshape(-1)
        target_qpos = np.asarray(ref_qpos, dtype=np.float64).reshape(-1)
        if target_qpos.size > current_qpos.size:
            target_qpos = target_qpos[-current_qpos.size:]
        n = min(current_qpos.size, target_qpos.size)
        if n == 0:
            return 0.0

        joint_error = np.mean((current_qpos[:n] - target_qpos[:n]) ** 2)
        return float(np.exp(-self.reward_specs.get("k_joint_angle", 5.0) * joint_error))

    def compute_muscle_drive_reward(self) -> float:
        """
        Penalizes excessive muscle activation.

        The simulator exposes muscle activations through `mj_data.ctrl` after the
        physics step. Lower activation receives a larger reward.
        """
        muscle_drive = np.asarray(self.mj_data.ctrl, dtype=np.float64)
        if muscle_drive.size == 0:
            return 0.0
        drive_cost = np.mean(muscle_drive ** 2)
        return float(np.exp(-self.reward_specs.get("k_muscle_drive", 1.0) * drive_cost))

    def compute_smoothness_reward(self, action: Optional[np.ndarray]) -> float:
        """
        Penalizes abrupt policy-output changes between control steps.
        """
        if action is None:
            return 0.0

        action = np.asarray(action, dtype=np.float64).reshape(-1)
        prev_action = getattr(self, "_prev_reward_action", None)
        self._prev_reward_action = action.copy()
        if prev_action is None or prev_action.shape != action.shape:
            return 1.0

        smoothness_cost = np.mean((action - prev_action) ** 2)
        return float(np.exp(-self.reward_specs.get("k_smoothness", 2.0) * smoothness_cost))
