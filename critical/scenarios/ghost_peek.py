# 场景7: 鬼探头 — 货车停在相邻车道前方, 行人从货车车头冲出, 自车急刹
import carla
from scenarios.base_scenario import BaseScenario
from utils.carla_utils import spawn_pedestrian_at


class GhostPeekScenario(BaseScenario):
    def __init__(self):
        super().__init__()
        self.name = "ghost_peek"
        ...
        self._trigger_step = int(2.3 * 20)       # 2.3s 行人冲出（确保自车刹停在行人前）
    ...
    def _spawn_obstacle_and_pedestrian(self):
        ...
        truck_sp = carla.Transform(
            carla.Location(x=truck_x, y=truck_y, z=ego_loc.z + 0.5),
            ego_tf.rotation)
        ...
        self._target_y = ego_loc.y - right.y * 5.0       # 对侧人行道
    ...
    def _control_loop(self):
        ...
        for tick in range(int(60 * 20)):
            ...
            if phase == "first_half" and self._ped:
                e = tick - phase_start
                if e >= self._cross_time:
                    self._set_ped_y(self._lane_y); phase, phase_start = "paused", tick
                else:
                    self._set_ped_y(ped_start_y + (self._lane_y - ped_start_y) * (e / self._cross_time))
            elif phase == "paused":
                ...
                e = tick - phase_start
                if e >= self._cross_time:
                    self._set_ped_y(self._target_y); phase = "done"
                else:
                    self._set_ped_y(ped_start_y + (self._target_y - ped_start_y) * (e / self._cross_time))
