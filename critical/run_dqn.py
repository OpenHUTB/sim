# experiments/run_dqn.py — 标准 DQN 训练入口
...
def run(scenario_name="rain_storm", max_episodes=MAX_EPISODES,
        render=False, model_dir="models/dqn"):
    ...
    print("智能体: DQN | 设备: %s" % agent.device)

    dirs = init_result_dir("dqn_%s" % scenario_name, base_dir="results/dqn")
    model_path = os.path.join(model_dir, "dqn_%s.pth" % scenario_name)
    ...
    for ep in range(1, max_episodes + 1):
        agent.start_episode()
        state = env.reset()                      # 1. env 生成自车 + 设天气
        scenario.spawn_scenario_actors(env)       # 2. 场景生成对抗车/行人/障碍物
    ...
    save_metrics(os.path.join(dirs["data"], "final_metrics.json"),
        {"scenario": scenario_name, "algorithm": "DQN", "episodes": max_episodes,
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="DQN 训练")
    p.add_argument("--scenario", type=str, default="rain_storm")
    p.add_argument("--episodes", type=int, default=MAX_EPISODES)
    p.add_argument("--render", action="store_true")
    p.add_argument("--model_dir", type=str, default="models/dqn")
    args = p.parse_args()
