% ============================================================
% 复现 figure_8.png — 雾天鬼探头场景训练结果对比
% 左图：训练奖励曲线    右图：评估指标柱状图
% 注意：原图中 Attention-DQN 训练曲线未能完整呈现
% ============================================================
clear; clc; close all;
...
% --- Attention-DQN (蓝色) 训练曲线不完整 ---
rng(44);
dq_ep = linspace(0, 500, 120);
dq_base = 30 + randn(size(dq_ep)) * 15;  % 散乱无规律
dq_base(dq_ep > 350) = NaN;              % 后期数据缺失
episodes_dqn = dq_ep;
rewards_dqn = dq_base;
...
dqn_values = [18.5, 35.2, 1.0, 1.8, 4.2];
ppo_values = [5.2, 88.5, 2.0, 4.2, 1.5];
