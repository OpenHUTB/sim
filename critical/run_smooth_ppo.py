# experiments/run_smooth_ppo.py — Smooth-PPO 训练入口（创新算法）
import sys, os, argparse
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from rl_algorithms.ppo import SmoothPPOAgent
...
DEFAULT_SCENARIOS = ["emergency_brake", "cut_in"]


def run(scenario_name="emergency_brake", max_episodes=MAX_EPISODES,
        render=False, model_dir="models/smooth_ppo"):
    scenario = create_scenario(scenario_name)
    env = scenario.create_env()
    print("场景: %s | 类别: %s" % (scenario.name, scenario.category))

    agent = SmoothPPOAgent(state_size=env.observation_space, action_size=env.action_space)
    print("智能体: Smooth-PPO | eps_clip: %.2f | smooth_alpha: %.3f"
          % (agent.eps_clip, agent.smooth_alpha))

    dirs = init_result_dir("smooth_ppo_%s" % scenario_name, base_dir="results/smooth_ppo")
    model_path = os.path.join(model_dir, "smooth_ppo_%s.pth" % scenario_name)
    ...
    save_metrics(os.path.join(dirs["data"], "final_metrics.json"),
        {"scenario": scenario_name, "algorithm": "Smooth-PPO", "episodes": max_episodes,
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Smooth-PPO 训练")
    p.add_argument("--scenario", type=str, default="emergency_brake", choices=DEFAULT_SCENARIOS + ["all"])
    p.add_argument("--episodes", type=int, default=MAX_EPISODES)
    p.add_argument("--render", action="store_true")
    p.add_argument("--model_dir", type=str, default="models/smooth_ppo")
    args = p.parse_args()
