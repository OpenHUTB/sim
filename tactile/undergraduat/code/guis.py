...
class TUI(GUI):
    ...
    MOVE_SCALE = 0.5
    HAND_MOVE_KEYS = {
        keyboard.KeyCode.from_char('w'): [1, 0, 0],
        keyboard.KeyCode.from_char('s'): [-1, 0, 0],
        keyboard.KeyCode.from_char('a'): [0, 1, 0],
        keyboard.KeyCode.from_char('d'): [0, -1, 0],
        keyboard.KeyCode.from_char('q'): [0, 0, 1],
        keyboard.KeyCode.from_char('e'): [0, 0, -1],
    }
    ...
    def _key_press(self, key):
        if key == keyboard.Key.esc:
            ...
            if hasattr(self._engine, "nudge_hand"):
                ...
                self._engine.nudge_hand(1, delta.tolist())
    ...
    def start_gui(self, engine: Engine, visualizer: Visualizer):
        ...
        print(f"Press on {Style.BRIGHT}WASD + QE{Style.NORMAL} to move the right hand.")
