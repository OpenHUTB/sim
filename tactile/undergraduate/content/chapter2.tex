...
质点-弹簧模型（Mass-Spring Model，MSM）是即时交互领域里最成熟、使用最广泛的可变形体建模方法\textsuperscript{{[}28{]}}，该模型的基本结构是由质点（质量分布）、弹簧（弹性约束）和阻尼器（振动衰减）这三部分组成。

\begin{center}
\includegraphics[width=0.88\linewidth,height=0.68\textheight,keepaspectratio]{images/image4.png}
\end{center}

\textbf{图2.1 质点--弹簧模型拓扑结构原理图}
...
$\sigma=E\varepsilon$ （2-1）

其中：σ为应力；E为弹性模量；\emph{ϵ}为应变。
...
$\sigma=E\varepsilon+\eta\dot{\varepsilon}$ （2-2）

其中：\emph{η}为粘性系数；$\dot{\varepsilon}$为应变速率。该模型能够准确还原``快按更硬、慢按更软''的真实生物组织触感。
...
其中，F是法向接触力，\emph{k}是等效刚度，\emph{δ}是变形或者穿透深度。
...
$F_{filtered}(t)=\frac{1}{N}\sum_{i=0}^{N-1}F(t-i\Delta t)$ （2-4）
...
$\dot{F}=\frac{F(t)-F(t-\Delta t)}{\Delta t}$ （2-5）
...
$I=I_0\times(0.5+0.5F_{factor})\times(0.3+0.7v_{factor})$ （2-6）
...
$\dot{q}=(J^TJ+\lambda^2I)^{-1}J^T\dot{x}$ (2-7)

其中：\emph{J}为雅可比矩阵；\emph{λ}为阻尼系数；$\dot{x}$为末端速度；$\dot{q}$为关节速度。

\begin{center}
\includegraphics[width=0.88\linewidth,height=0.68\textheight,keepaspectratio]{images/image15.png}
\end{center}

\textbf{图2.2 DLS 逆运动学求解流程图}
...
MuJoCo是目前全世界最符合触觉交互、机器人即时仿真物理引擎、高精度、高稳定、低时延、接触求解能力强的机器人仿真软件\textsuperscript{{[}30{]}}。
